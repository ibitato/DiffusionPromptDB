...
Respond with ONLY the art style name, nothing else."""
    ...
    def prepare_batch_input(self, input_file):
        ...
        output_file = Path("data") / f"batch_input_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
    ...
    def run(self, input_file, dry_run=False):
        ...
        try:
            ...
            s3_input_file = f"{input_s3_uri.rstrip('/')}/art-cataloger-{timestamp}.jsonl"
...
def main():
    ...
    parser.add_argument("--input", default="data/unspecified_prompts_*.jsonl",
                       help="Archivo JSONL con los prompts a catalogar")
    parser.add_argument("--config", default="config.yaml",
                       help="Archivo de configuración")
    parser.add_argument("--dry-run", action="store_true",
                       help="Preparar input sin enviar a Bedrock")
