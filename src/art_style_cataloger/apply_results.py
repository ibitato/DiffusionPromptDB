...
def download_results(job_arn, config):
    ...
    output_file = results_dir / f"batch_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl"
                ...
                if content and len(content) > 0:
                    ...
                    if text.startswith('{'):
                        try:
                            ...
                            primary_style = art_style_data.get('primary_style', 'unspecified')
...
def backup_database(db_path):
    ...
    backup_path = Path(str(db_path) + f".backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    print(f"\n💾 Creando backup: {backup_path}")
    shutil.copy2(db_path, backup_path)
    print(f"✅ Backup creado")
    return backup_path

def update_database(results, db_path="../api/database/prompts_catalog.db", dry_run=False):
...
def main():
    ...
    parser.add_argument("--results-file", help="Archivo de resultados (si ya fue descargado)")
    ...
    if errors:
        ...
        errors_file = Path("results") / f"errors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
