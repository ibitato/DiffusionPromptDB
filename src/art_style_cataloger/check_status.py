...
def check_status(job_arn=None, watch=False):
    ...
    if not job_arn:
        ...
        if not job_file:
            print(
                "❌ No se encontró ningún job. Ejecuta primero: python run_art_cataloger.py"
            )
            return
    ...
    while True:
        try:
            ...
            response = bedrock.bedrock_client.get_model_invocation_job(
            ...
            if submit_time:
                elapsed = (
                    datetime.now(submit_time.tzinfo) - submit_time
                ).total_seconds()
            else:
                elapsed = 0
