...
class CatalogTester:
    ...
    def test_basic_stats(self):
        ...
        self.test("NSFW levels populated", len(nsfw_dist) >= 3, 
                 f"Found {len(nsfw_dist)} NSFW levels")
        
        # Character data
        char_count = cursor.execute("SELECT COUNT(*) FROM characters").fetchone()[0]
        self.test("Character data exists", char_count > 10000, 
                 f"{char_count:,} character records")
        
        # Main tags
        tag_count = cursor.execute("SELECT COUNT(DISTINCT tag) FROM main_tags").fetchone()[0]
        self.test("Tags extracted", tag_count > 1000, 
                 f"{tag_count:,} unique tags")
    ...
    def test_nsfw_search(self):
        ...
        """).fetchone()[0]
        self.test("Search explicit content", explicit > 5000, 
        ...
        """).fetchone()[0]
        self.test("Search safe content", safe > 0, 
                 f"Found {safe:,} safe prompts")
        
        # Search suggestive
        suggestive = cursor.execute("""
            SELECT COUNT(*) FROM nsfw_content WHERE level = 'suggestive'
        """).fetchone()[0]
        self.test("Search suggestive content", suggestive > 0, 
    ...
    def test_character_search(self):
        ...
        """).fetchone()[0]
        self.test("Search solo characters", solo > 5000, 
                 f"Found {solo:,} solo prompts")
        
        # Couples
        couples = cursor.execute("""
            SELECT COUNT(*) FROM characters WHERE number_of_people = 2
        """).fetchone()[0]
        self.test("Search couples", couples > 3000, 
                 f"Found {couples:,} couple prompts")
        
        # Groups
        groups = cursor.execute("""
            SELECT COUNT(*) FROM characters WHERE number_of_people >= 3
        """).fetchone()[0]
        self.test("Search groups", groups > 400, 
                 f"Found {groups:,} group prompts")
    ...
    def test_art_style_search(self):
        ...
        """).fetchone()[0]
        self.test("Search anime style", anime > 1000, 
                 f"Found {anime:,} anime prompts")
        
        # Realistic
        realistic = cursor.execute("""
            SELECT COUNT(*) FROM art_styles WHERE primary_style = 'realistic'
        """).fetchone()[0]
        self.test("Search realistic style", realistic > 2000, 
        ...
        """).fetchone()[0]
        self.test("Search photorealistic style", photo > 0, 
    ...
    def test_tag_search(self):
        ...
        """).fetchone()[0]
        self.test("Search '1girl' tag", girl_tag > 5000, 
        ...
        """).fetchone()[0]
        self.test("Search 'masterpiece' tag", masterpiece > 3000, 
        ...
        """).fetchone()[0]
        self.test("Search 'nude' tag", nude > 2000, 
                 f"Found {nude:,} prompts with 'nude'")
    ...
    def test_complex_search(self):
        ...
        """).fetchone()[0]
        self.test("Complex: explicit + 1 girl + anime", result1 > 100, 
        ...
        """).fetchone()[0]
        self.test("Complex: realistic + couple", result2 > 100, 
        ...
        result3 = cursor.execute("""
            ...
            AND t.tag = 'long hair'
        """).fetchone()[0]
        self.test("Complex: suggestive + long hair", result3 > 10, 
    ...
    def test_data_integrity(self):
        ...
        self.test("All prompts classified for NSFW", prompts_total == nsfw_total, 
        ...
        self.test("All prompts have character data", char_total == prompts_total, 
                 f"{char_total:,}/{prompts_total:,} have character data")
        
        # Main tags populated
        tags_total = cursor.execute("SELECT COUNT(*) FROM main_tags").fetchone()[0]
        self.test("Main tags extracted", tags_total > 100000, 
        ...
        """).fetchone()[0]
        self.test("No orphaned NSFW records", orphaned == 0, 
    ...
    def test_indexes(self):
        ...
        self.test("Character gender index exists", 'idx_character_gender' in index_names)
        self.test("Main tags index exists", 'idx_main_tags' in index_names)
        self.test("People count index exists", 'idx_characters_people_count' in index_names)
    ...
    def test_sample_queries(self):
        ...
        self.test("Query returns results", len(result) > 0, 
