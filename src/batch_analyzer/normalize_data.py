...
def normalize_prompt_data(result: dict) -> dict:
    ...
    sexual["genital_visibility"] = normalize_string_field(
        sexual.get("genital_visibility")
    )

    relationships = categories.get("relationships", {})
    relationships["interaction_type"] = normalize_string_field(
        relationships.get("interaction_type")
    )
    relationships["relationship"] = normalize_string_field(
        relationships.get("relationship")
    )
    relationships["pov_perspective"] = normalize_string_field(
        relationships.get("pov_perspective")
    )
...
def normalize_file(input_file: str, output_file: str):
    ...
    with jsonlines.open(input_file) as reader, jsonlines.open(
        ...
        for idx, result in enumerate(reader, start=1):
            original_nsfw = (
                result.get("categories", {}).get("nsfw_content", {}).get("level")
            )
            ...
            new_nsfw = (
                normalized_result.get("categories", {})
                .get("nsfw_content", {})
                .get("level")
            )
...
def main():
    ...
    print(f"\n📋 Next steps:")
    print(
        f"   1. Import: python import_to_db.py {args.output} --db prompts_catalog.db --stats"
    )
