...
class PromptDatabase:
    ...
    def import_prompt(self, result: dict):
        ...
        """,
            (
                prompt_id,
                char.get("number_of_people", 0),
                char.get("breast_size", "unspecified"),
            ),
        )

        for gender in char.get("genders", []):
            self.conn.execute(
                "INSERT INTO character_genders VALUES (?, ?)", (prompt_id, gender)
            )

        for age in char.get("age_ranges", []):
            self.conn.execute(
                "INSERT INTO character_ages VALUES (?, ?)", (prompt_id, age)
            )

        for body_type in char.get("body_types", []):
            self.conn.execute(
                "INSERT INTO character_body_types VALUES (?, ?)", (prompt_id, body_type)
            )
        ...
        for color in hair.get("colors", []):
            self.conn.execute(
                "INSERT INTO character_hair (prompt_id, color) VALUES (?, ?)",
                (prompt_id, color),
            )
        for style in hair.get("styles", []):
            self.conn.execute(
                "INSERT INTO character_hair (prompt_id, style) VALUES (?, ?)",
                (prompt_id, style),
            )
        for length in hair.get("lengths", []):
            self.conn.execute(
                "INSERT INTO character_hair (prompt_id, length) VALUES (?, ?)",
                (prompt_id, length),
            )
        ...
        for color in eyes.get("colors", []):
            self.conn.execute(
                "INSERT INTO character_eyes (prompt_id, color) VALUES (?, ?)",
                (prompt_id, color),
            )
        for shape in eyes.get("shapes", []):
            self.conn.execute(
                "INSERT INTO character_eyes (prompt_id, shape) VALUES (?, ?)",
                (prompt_id, shape),
            )
        ...
        for action in pose.get("actions", []):
            self.conn.execute(
                "INSERT INTO pose_actions VALUES (?, ?)", (prompt_id, action)
            )
        ...
        for item in clothing.get("items", []):
            self.conn.execute(
                "INSERT INTO clothing_items VALUES (?, ?)", (prompt_id, item)
            )
        for accessory in clothing.get("accessories", []):
            self.conn.execute(
                "INSERT INTO clothing_accessories VALUES (?, ?)", (prompt_id, accessory)
            )
        ...
        """,
            (
                prompt_id,
                setting.get("location_type"),
                setting.get("indoor_outdoor"),
                setting.get("specific_place"),
            ),
        )

        for detail in setting.get("environment_details", []):
            self.conn.execute(
                "INSERT INTO environment_details VALUES (?, ?)", (prompt_id, detail)
            )
        ...
        for quality in lighting.get("quality", []):
            self.conn.execute(
                "INSERT INTO lighting_quality VALUES (?, ?)", (prompt_id, quality)
            )
        ...
        for tag in art_style.get("quality_tags", []):
            self.conn.execute(
                "INSERT INTO art_style_tags VALUES (?, ?, ?)",
                (prompt_id, "quality", tag),
            )
        for technique in art_style.get("technique", []):
            self.conn.execute(
                "INSERT INTO art_style_tags VALUES (?, ?, ?)",
                (prompt_id, "technique", technique),
            )
        for score in art_style.get("score_indicators", []):
            self.conn.execute(
                "INSERT INTO art_style_tags VALUES (?, ?, ?)",
                (prompt_id, "score", score),
            )

        # Technical
        self.conn.execute(
            "INSERT OR REPLACE INTO technical (prompt_id) VALUES (?)", (prompt_id,)
        )

        technical = categories.get("technical", {})
        for res in technical.get("resolution", []):
            self.conn.execute(
                "INSERT INTO technical_details VALUES (?, ?, ?)",
                (prompt_id, "resolution", res),
            )
        for setting in technical.get("camera_settings", []):
            self.conn.execute(
                "INSERT INTO technical_details VALUES (?, ?, ?)",
                (prompt_id, "camera_setting", setting),
            )
        for detail in technical.get("detail_level", []):
            self.conn.execute(
                "INSERT INTO technical_details VALUES (?, ?, ?)",
                (prompt_id, "detail_level", detail),
            )
        ...
        for element in nsfw.get("elements", []):
            self.conn.execute(
                "INSERT INTO nsfw_elements VALUES (?, ?)", (prompt_id, element)
            )
        ...
        for act in sexual.get("sexual_acts", []):
            self.conn.execute(
                "INSERT INTO sexual_details VALUES (?, ?, ?)", (prompt_id, "act", act)
            )
        for position in sexual.get("sexual_positions", []):
            self.conn.execute(
                "INSERT INTO sexual_details VALUES (?, ?, ?)",
                (prompt_id, "position", position),
            )
        for fluid in sexual.get("body_fluids", []):
            self.conn.execute(
                "INSERT INTO sexual_details VALUES (?, ?, ?)",
                (prompt_id, "body_fluid", fluid),
            )
        for fetish in sexual.get("fetishes", []):
            self.conn.execute(
                "INSERT INTO sexual_details VALUES (?, ?, ?)",
                (prompt_id, "fetish", fetish),
            )
        ...
        """,
            (
                prompt_id,
                rel.get("interaction_type"),
                rel.get("relationship"),
                rel.get("pov_perspective"),
            ),
        )
        ...
        for char_name in refs.get("character_names", []):
            self.conn.execute(
                "INSERT INTO prompt_references VALUES (?, ?, ?)",
                (prompt_id, "character", char_name),
            )
        for series in refs.get("series_franchise", []):
            self.conn.execute(
                "INSERT INTO prompt_references VALUES (?, ?, ?)",
                (prompt_id, "series", series),
            )
        for artist in refs.get("artist_references", []):
            self.conn.execute(
                "INSERT INTO prompt_references VALUES (?, ?, ?)",
                (prompt_id, "artist", artist),
            )
        ...
        """,
            (
                ...
                camera.get("focus_area"),
            ),
        )

        for note in camera.get("composition_notes", []):
            self.conn.execute(
                "INSERT INTO composition_notes VALUES (?, ?)", (prompt_id, note)
            )
        ...
        for emotion in mood.get("emotions", []):
            self.conn.execute(
                "INSERT INTO emotions VALUES (?, ?)", (prompt_id, emotion)
            )

        # Main tags
        for idx, tag in enumerate(categories.get("main_tags", [])):
            self.conn.execute(
                "INSERT INTO main_tags VALUES (?, ?, ?)", (prompt_id, tag, idx)
            )
    ...
    def get_stats(self):
        ...
        stats = {
            "total_prompts": cursor.execute("SELECT COUNT(*) FROM prompts").fetchone()[
                0
            ],
            "nsfw_distribution": {},
            "top_tags": [],
        ...
        for row in cursor.execute(
            "SELECT level, COUNT(*) FROM nsfw_content GROUP BY level"
        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import analyzed prompts to SQLite database"
    )
    ...
    parser.add_argument(
        "--stats", action="store_true", help="Show statistics after import"
    )
