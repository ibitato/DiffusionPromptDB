    ...
    def generate_statistics(self, results: List[Dict]) -> Dict:
        ...
        for result in results:
            try:
                ...
                # Art styles
                art_style = categories.get('art_style', {}).get('primary_style', 'unknown')
                stats['top_art_styles'][art_style] = stats['top_art_styles'].get(art_style, 0) + 1
                
                # Character counts
                char_count = categories.get('character', {}).get('count', 0)
                stats['character_counts'][str(char_count)] = stats['character_counts'].get(str(char_count), 0) + 1
        ...
        # Sort and limit
        stats['top_art_styles'] = dict(sorted(stats['top_art_styles'].items(), 
                                             key=lambda x: x[1], reverse=True)[:10])
        stats['common_tags'] = dict(sorted(stats['common_tags'].items(), 
