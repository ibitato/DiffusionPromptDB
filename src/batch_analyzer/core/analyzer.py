...
class PromptAnalyzer:
    ...
    def __init__(
        ...
        self.processor = BatchProcessor(input_file=input_file, batch_size=batch_size)
    ...
    def create_batch_requests(self, prompts: List[Dict]) -> List[Dict]:
        """
    ...
    def process_batch_results(
        self, batch_results: List[Dict], original_prompts: Dict[int, str]
    ) -> List[Dict]:
        """
    ...
    def analyze_prompts(
        ...
        try:
            ...
            logger.info(
                f"Cost estimate: ${cost_estimate['total_cost_usd']} USD for {prompts_to_process} prompts"
            )
            ...
            if s3_bucket:
                input_s3_uri = (
                    f"s3://{s3_bucket}/batch-analyzer/input/{timestamp}/input.jsonl"
                )
                ...
                logger.warning(
                    f"No S3 bucket specified. You'll need to create: {bucket_name}"
                )
                return (
                    False,
                    "S3 bucket must be specified. Please configure s3_bucket in config.",
                )
            ...
            for prompt in self.processor.read_prompts(
                start_id=resume_from_id, max_count=max_count
            ):
            ...
            success, msg = self.bedrock_client.upload_batch_input(
                batch_requests, input_s3_uri
            )
            if not success:
    ...
    def retrieve_results(self, job_info_file: str) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            success, batch_results, msg = self.bedrock_client.download_batch_output(
                output_s3_uri
            )
            if not success:
            ...
            processed_results = self.process_batch_results(
                batch_results, original_prompts
            )
