...
class BedrockBatchClient:
    ...
    def test_connection(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if models:
                return (
                    True,
                    f"Successfully connected. Found {len(models)} models available.",
                )
            else:
    ...
    def check_model_available(self, model_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                True,
                f"Model {model_id} is available. Status: {model_info.get('modelLifecycle', {}).get('status', 'unknown')}",
            )
        ...
        try:
            ...
            if not role_arn:
                try:
                    ...
                    role_arn = (
                        f"arn:aws:iam::{account_id}:role/BedrockBatchAnalyzerRole"
                    )
    ...
    def get_job_status(self, job_arn: str) -> Tuple[bool, Optional[Dict], str]:
        ...
        try:
            ...
            return (
                False,
                None,
                f"Error getting job status: {e.response['Error']['Message']}",
            )
        except Exception as e:
            return False, None, f"Unexpected error: {str(e)}"

    def wait_for_job_completion(
        self, job_arn: str, check_interval: int = 300, timeout: int = 86400
    ...
    def list_batch_jobs(
        ...
        try:
            response = self.bedrock_client.list_model_invocation_jobs(
                maxResults=max_results, sortBy="SubmitTime", sortOrder="Descending"
            )
