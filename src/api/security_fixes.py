                ...
                for i, line in enumerate(lines, 1):
                    if 'password' in line.lower() and '=' in line and '"' in line:
                        if 'example' not in line.lower() and 'test' not in line.lower():
                            issues['critical'].append(f"Hardcoded password in {file}:{i}")
                ...
                for i, line in enumerate(lines, 1):
                    if 'secret' in line.lower() and '=' in line and '"' in line:
                        if 'your-secret-key' in line or 'change-this' in line:
                            issues['critical'].append(f"Default secret key in {file}:{i}")
                        elif 'example' not in line.lower():
                            issues['high'].append(f"Potential hardcoded secret in {file}:{i}")
            ...
            if 'execute' in content:
                ...
                for i, line in enumerate(lines, 1):
                    if 'execute' in line and 'f"' in line:
                        issues['high'].append(f"Potential SQL injection (f-string) in {file}:{i}")
                    if 'execute' in line and '.format(' in line:
                        issues['high'].append(f"Potential SQL injection (format) in {file}:{i}")
                    if 'execute' in line and ' + ' in line:
                        issues['high'].append(f"Potential SQL injection (concatenation) in {file}:{i}")
...
def check_database_security():
    ...
    try:
        ...
        if cursor.fetchone():
            ...
            if count > 0:
                issues.append(f"Found {count} users with potentially unhashed passwords")
...
def display_report(issues):
    ...
    if total_issues == 0:
        ...
        print("   Note: This is a basic check. Professional security audit recommended.")
    else:
