...
class APITester:
    ...
    def test_server_running(self):
        ...
        try:
            ...
            self.test(
                "Server is responding", False, "Server not running at localhost:8000"
            )
            return False
    ...
    def test_prompts_endpoints(self):
        ...
        response = requests.get(
            f"{self.base_url}/api/v1/prompts", headers={"X-API-Key": self.api_key}
        )
        ...
        self.test(
            "POST /prompts (API key not enough)", response.status_code in [401, 403]
        )
        ...
        if self.token:
            response = requests.post(
                f"{self.base_url}/api/v1/prompts",
                json={"text": "automated test prompt", "category": "test", "rating": 5},
                headers={"Authorization": f"Bearer {self.token}"},
            )
            self.test(
                "POST /prompts (with JWT)", response.status_code in [200, 201, 400, 422]
            )
    ...
    def test_catalog_endpoints(self):
        ...
        if response.status_code == 200:
            data = response.json()
            self.test(
                "GET /catalog/search/nsfw/explicit",
                True,
                f"Found {data.get('total', 0)} results",
            )
        elif response.status_code == 503:
            self.test(
                "GET /catalog/search/nsfw/explicit",
                True,
                "Catalog DB not available (OK if not set up)",
            )
        else:
            self.test(
                "GET /catalog/search/nsfw/explicit",
        ...
        if response.status_code == 200:
            data = response.json()
            self.test(
                "GET /catalog/search/style/anime",
                True,
                f"Found {data.get('total', 0)} results",
            )
        else:
    ...
    def test_search_endpoints(self):
        ...
        if response.status_code == 200:
            data = response.json()
            self.test(
                "GET /search/complex", True, f"Found {data.get('total', 0)} results"
            )
        else:
            self.test(
                "GET /search/complex",
        ...
        if response.status_code == 200:
            data = response.json()
            self.test(
                "GET /search/tags/1girl", True, f"Found {data.get('total', 0)} results"
            )
        else:
            self.test(
                "GET /search/tags/1girl",
                response.status_code == 503,
                "Catalog DB not available",
            )
    ...
    def test_security(self):
        ...
        response = requests.get(
            f"{self.base_url}/api/v1/prompts", headers={"X-API-Key": "invalid-key"}
        )
